...
import { createJob } from "@/lib/actions/recruter-jobs";
...
export default function PostJobForm({company}) {
    ...
    const handleSubmit = async (e) => {
        ...
        console.log(payload,'this is payload of new job')
        
        const res = await createJob(payload)
        if(res.insertedId){
            alert('Job post Successfully')
            redirect('/dashboard/recruter/jobs')
            e.target.reset()
            setIsRemote(false)
        }
    ...
    return (
        <div className="min-h-screen bg-[#0d0d0e] text-white py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto bg-[#121214] border border-zinc-900 rounded-xl p-8 shadow-2xl">
                ...
                {
                    company.status !== 'Approved' ? (<div> You job submisition is pending && waiting for admin approved  </div>) : (<Form onSubmit={handleSubmit} className="space-y-8" validationErrors={errors} validationBehavior='aria'>
